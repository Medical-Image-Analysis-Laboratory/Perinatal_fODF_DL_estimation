...
def davood_net_dti_unc(X, ks_0, depth, n_feat_0, n_channel, n_class, p_keep_conv, bias_init=0.001):
    feat_fine = [None] * (depth - 1)
    for level in range(depth):
        ks = ks_0
        if level == 0:
    ...
    # output= tf.nn.dropout(output, p_keep_conv)
    ks = 1
    # n_feat_v= [n_channel, 32, 32, 64, 64, 64, 32, 32, 16, 16, 1]
    # n_feat_v= [n_channel, 128, 128, 256, 256, 256, 256, 128, 128, 16, 16, 1]
    n_feat_v = [n_channel, 32, 32, 32, 32, 16, 16, 8, 8, 1]
    # n_feat_v= [n_channel, 32, 32, 32, 32, 8, 8, 1]
    inp = X
    for i in range(len(n_feat_v) - 1):
        n_feat_in = n_feat_v[i]
        n_feat_out = n_feat_v[i + 1]
        W_2 = tf.Variable(tf.truncated_normal([ks, ks, ks, n_feat_in, n_feat_out], stddev=s_dev), name=name_w)
        b_2 = tf.Variable(tf.constant(bias_init, shape=[n_feat_out]), name=name_b)
        if i < len(n_feat_v) - 2:
            # print(i, n_feat_in, n_feat_out)
            out = tf.nn.relu(tf.add(tf.nn.conv3d(inp, W_2, strides=[1, 1, 1, 1, 1], padding='SAME'), b_2))
            if n_feat_in == n_feat_out:
                out = out + inp
            inp = tf.nn.dropout(out, p_keep_conv)
        else:
            output_s = tf.add(tf.nn.conv3d(inp, W_2, strides=[1, 1, 1, 1, 1], padding='SAME'), b_2)
    return output, output_s
...
def davood_net_2d(X, ks_0, depth, n_feat_0, n_channel, n_class, p_keep_conv, bias_init=0.0):
    ...
    for level in range(depth):
        ...
        if level == 0:
            ...
            W_1 = tf.Variable(tf.constant(0.1, shape=[ks, ks, n_channel, n_feat_0]), name=name_w)
            b_1 = tf.Variable(tf.constant(0.1, shape=[n_feat_0]), name=name_b)
            inp00 = X
            # inp = tf.add(   X, b_1)
            inp = tf.add(tf.nn.conv2d(X, W_1, strides=[1, strd, strd, 1], padding='SAME'), b_1)
    ...
    #                 b_1 = tf.Variable(tf.constant(bias_init, shape=[n_feat]), name=name_b)
    #                 inp_0 = tf.nn.relu(tf.add(tf.nn.conv2d(inp_0, W_1, strides=[1, strd, strd, 1], padding='SAME'), b_1))
    #                 # inp_0 = tf.nn.dropout(inp_0, p_keep_conv)
    ...
    output = X
    output_s = X
    return output, output_s, feat_fine, inp00, inp11
...
def davood_3d_cae(X, ks_0, n_feat_vec, n_enrich_vec, strd_vec, n_channel, bias_init=0.001):
    ...
    for i in range(len(n_feat_vec) - 1, -1, -1):
        ...
        if i > 0:
            inp = tf.nn.relu(
                tf.add(tf.nn.conv3d_transpose(inp, W_1, out_shape, strides=[1, strd, strd, strd, 1], padding='SAME'),
                       b_1))
        else:
            inp = tf.add(tf.nn.conv3d_transpose(inp, W_1, out_shape, strides=[1, strd, strd, strd, 1], padding='SAME'),
                         b_1)
        if n_enrich_vec[i] > 0:
            inp0 = inp
            if i == 0:
            ...
            for j in range(n_enrich_vec[i]):
                ...
                if i > 0:
                    inp = tf.nn.relu(
                        tf.add(tf.nn.conv3d(inp, W_1, strides=[1, strd, strd, strd, 1], padding='SAME'), b_1))
                else:
...
def cost_dice_multi_forground_generalized(Y, predicter, weight_vec=np.ones(2), n_foreground=2, loss_type='sorensen',
                                          smooth=1e-5):
    cost = n_foreground
    for i_f in range(n_foreground):
        target = Y[:, :, :, :, i_f + 1]
        ...
        if loss_type == 'jaccard':
            ...
            l = tf.reduce_sum(output)
            r = tf.reduce_sum(target)
        dice = (2. * inse * weight_vec[i_f - 1] + smooth) / ((l + r) * weight_vec[i_f - 1] + smooth)
        # dice_whole = tf.reduce_mean(dice)
        cost -= dice
    return cost
...
def davood_reg_net(X, n_feat_vec, p_keep_hidden, bias_init=0.001):
    ...
    for i in range(len(n_feat_vec) - 1):
        n_feat_in = n_feat_vec[i]
        n_feat_out = n_feat_vec[i + 1]
        W_fc = tf.Variable(tf.truncated_normal([n_feat_in, n_feat_out], stddev=np.sqrt(2.0 / n_feat_out)),
                           name='W_fc_' + str(i))
...
def davood_net_return_fmaps(X, ks_0, depth, n_feat_0, n_channel, n_class, p_keep_conv, bias_init=0.001,
...
def davood_net_frcnn(X, ks_0, depth, n_feat_0, n_channel, n_class, n_anchor, p_keep_conv, bias_init=0.001,
                     write_summaries=False):
    feat_fine = [None] * (depth - 1)
    if write_summaries:
        tf.summary.image('input image', slice_and_scale(X))
...
def DCI_RESCNN(X, n_feat_vec, n_res_vec, ks_vec, strd_vec, n_channel, n_class, p_keep_conv, bias_init=0.0):
    ...
    for level in range(len(n_feat_vec) - 1):
        ...
        for i_res in range(n_res):
            ...
            b_1 = tf.Variable(tf.constant(bias_init, shape=[n_feat_out]), name=name_b)
            inp_res = tf.nn.relu(
                tf.add(tf.nn.conv3d(inp_res, W_1, strides=[1, strd, strd, strd, 1], padding='SAME'), b_1))
...
def ViT_seg(X, T, n_patch, d_patch, d_emb, ViT_depth, n_head, d_emb_h, n_class=2, use_token=False,
    ...
    for i_depth in range(ViT_depth):
        ...
        for i_head in range(n_head):
            W_q = tf.Variable(tf.truncated_normal([n_feat_in, n_feat_out], stddev=np.sqrt(w_std / n_feat_out)),
                              name='W_q_' + str(i_depth) + '_' + str(i_head))
            Q = tf.matmul(inp, W_q)
            W_k = tf.Variable(tf.truncated_normal([n_feat_in, n_feat_out], stddev=np.sqrt(w_std / n_feat_out)),
                              name='W_k_' + str(i_depth) + '_' + str(i_head))
            K = tf.matmul(inp, W_k)
            W_v = tf.Variable(tf.truncated_normal([n_feat_in, n_feat_out], stddev=np.sqrt(w_std / n_feat_out)),
                              name='W_v_' + str(i_depth) + '_' + str(i_head))
            V = tf.matmul(inp, W_v)
            A = tf.nn.softmax(tf.matmul(Q, K, transpose_b=True) / d_emb_h ** 0.5, axis=-1,
                              name='A_' + str(i_depth) + '_' + str(i_head))
        ...
        n_feat_in = d_emb
        n_feat_out = d_emb
        W_l = tf.Variable(tf.truncated_normal([n_feat_in, n_feat_out], stddev=np.sqrt(w_std / n_feat_out)),
                          name='W_L_' + str(i_depth))
    ...
    if use_token:
        ...
        n_feat_in = d_emb
        n_feat_out = n_class
        W_oout = tf.Variable(tf.truncated_normal([n_feat_in, n_feat_out], stddev=np.sqrt(2.0 / n_feat_out)),
                             name='W_L_out')
...
def ViT_det_full_patch(X, T, pos_emb, n_patch, d_patch, d_patch_2, d_emb, ViT_depth, n_head, d_emb_h, n_class=2,
                       use_token=False,
                       residuals=True, layer_norm=True, learn_embd=False, p_keep=1.0, w_std=0.002):
    A_list = [None] * (ViT_depth * n_head)
    inp = X
...
def DTI_Transformer_plusplus(X, T, pos_emb, Spos_emb, Dpos_emb, n_inp, d_emb, ViT_depth, n_head, d_emb_h, n_channel, L,
                             n_class=6, use_token=False,
                             residuals=True, layer_norm=True, learn_embd=False, p_keep=1.0, w_std=0.002):
    A_list = [None] * (ViT_depth * n_head)
    inp = X
    n_feat_in = n_channel
    ...
    for i_depth in range(2):
        ...
        for i_head in range(n_head):
            W_q = tf.Variable(tf.truncated_normal([n_feat_in, n_feat_out], stddev=np.sqrt(w_std / n_feat_out)),
                              name='SW_q_' + str(i_depth) + '_' + str(i_head))
            Q = tf.matmul(S, W_q)
            Q = tf.nn.dropout(Q, p_keep)
            W_k = tf.Variable(tf.truncated_normal([n_feat_in, n_feat_out], stddev=np.sqrt(w_std / n_feat_out)),
                              name='SW_k_' + str(i_depth) + '_' + str(i_head))
            K = tf.matmul(S, W_k)
            K = tf.nn.dropout(K, p_keep)
            W_v = tf.Variable(tf.truncated_normal([n_feat_in, n_feat_out], stddev=np.sqrt(w_std / n_feat_out)),
                              name='SW_v_' + str(i_depth) + '_' + str(i_head))
            V = tf.matmul(S, W_v)
            V = tf.nn.dropout(V, p_keep)
            A = tf.nn.softmax(tf.matmul(Q, K, transpose_b=True) / d_emb_h ** 0.5, axis=-1,
                              name='SA_' + str(i_depth) + '_' + str(i_head))
            # A= tf.matmul(Q, K, transpose_b=True)
            # A_list[i_depth*n_head+i_head]= A
            SA = tf.matmul(A, V)
            if i_head == 0:
    ...
    for i_depth in range(2):
        ...
        for i_head in range(n_head):
            W_q = tf.Variable(tf.truncated_normal([n_feat_in, n_feat_out], stddev=np.sqrt(w_std / n_feat_out)),
                              name='DW_q_' + str(i_depth) + '_' + str(i_head))
            Q = tf.matmul(D, W_q)
            Q = tf.nn.dropout(Q, p_keep)
            W_k = tf.Variable(tf.truncated_normal([n_feat_in, n_feat_out], stddev=np.sqrt(w_std / n_feat_out)),
                              name='DW_k_' + str(i_depth) + '_' + str(i_head))
            K = tf.matmul(D, W_k)
            K = tf.nn.dropout(K, p_keep)
            W_v = tf.Variable(tf.truncated_normal([n_feat_in, n_feat_out], stddev=np.sqrt(w_std / n_feat_out)),
                              name='DW_v_' + str(i_depth) + '_' + str(i_head))
            V = tf.matmul(D, W_v)
            V = tf.nn.dropout(V, p_keep)
            A = tf.nn.softmax(tf.matmul(Q, K, transpose_b=True) / d_emb_h ** 0.5, axis=-1,
                              name='DA_' + str(i_depth) + '_' + str(i_head))
            # A= tf.matmul(Q, K, transpose_b=True)
            # A_list[i_depth*n_head+i_head]= A
            SA = tf.matmul(A, V)
            if i_head == 0:
...
def style_transfer_cae(X, ks_0, n_feat_vec, n_enrich_vec, strd_vec, n_channel, p_keep_conv, bias_init=0.001):
    ...
    for i in range(len(n_feat_vec) - 1, -1, -1):
        ...
        if i > 0:
            inp = tf.nn.relu(
                tf.add(tf.nn.conv2d_transpose(inp, W_1, out_shape, strides=[1, strd, strd, 1], padding='SAME'), b_1))
        else:
...
def davood_net_2d_style_transfer(X, ks_0, depth, n_feat_0, n_channel, n_class, p_keep_conv, bias_init=0.0):
    F_list = [None] * (depth * 2)
    i_F_list = -1
    feat_fine = [None] * (depth - 1)
    for level in range(depth):
...
def hist_classify_2d_return_fmaps(X, ks_0, n_feat_vec, n_enrich_vec, n_channel, n_class, p_keep_conv, bias_init=0.001):
    fmaps = [None] * (3)
    fmaps_i = 0
    inp = X
    for i in range(len(n_feat_vec)):
...
def volume_to_affine(X, ks_0, n_feat_vec, n_enrich_vec, n_channel, p_keep_conv, bias_init=0.001):
    inp = X
    for i in range(len(n_feat_vec)):
        if i == 0:
            n_feat_in = n_channel
